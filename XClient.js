...
function workflowAction() {
   ...
   {  
	   ...
	   else
	  {
       ...
       if (approver_rec =='project')
      {
           var projrec = nlapiLoadRecord('job',rec.getFieldValue('custbody_pp_er_project'));

           nlapiLogExecution('DEBUG','res cnt'+projrec.getLineItemCount('jobresources'));
	
	for ( var i = 1; i <= projrec.getLineItemCount('jobresources'); i++)
	{
                        if ( projrec.getLineItemValue('jobresources','role',i) == 'Project Manager')
                            {
                             return  projrec.getLineItemValue('jobresources','jobresource',i);          
                            }
                     }
